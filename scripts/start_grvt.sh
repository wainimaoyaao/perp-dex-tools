...
setup_logging() {
    # 使用新的日志准备函数
    prepare_log_file "$GRVT_LOG_FILE" "GRVT"
}
...
echo -e "${CYAN}日志输出: $GRVT_LOG_FILE (模式: ${redirect_symbol})${NC}"

# 在日志文件中添加启动标记
echo "=== $(date '+%Y-%m-%d %H:%M:%S') - GRVT Bot Starting (PID: $$) ===" $redirect_symbol "$GRVT_LOG_FILE"

nohup $START_CMD $redirect_symbol "$GRVT_LOG_FILE" 2>&1 &
...
if ps -p $GRVT_PID > /dev/null 2>&1; then
    ...
    if [ -f "$GRVT_LOG_FILE" ]; then
        sleep 2
        init_status=$(tail -10 "$GRVT_LOG_FILE" | grep -i "initialized\|started\|ready\|connected" | tail -1)
        if [ -n "$init_status" ]; then
            echo -e "${CYAN}状态: $init_status${NC}"
        fi
        
        # 检查是否有错误
        errors=$(tail -10 "$GRVT_LOG_FILE" | grep -i "error\|exception\|failed" | wc -l)
